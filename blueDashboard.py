...
destination = orgDestCSV.drop(columns=["Origin", "Start ID", "Start Latitude", "Start Longtitude"])
...
destination = destination.iloc[:,[0,2,3,1,4,5,6,7]]
destination.columns
...
origin_destination.columns
origin_destination = origin_destination.iloc[:,[0,3,2,5,6,7,1,4]]

origin_destination.isnull().sum()
